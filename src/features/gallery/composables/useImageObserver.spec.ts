...
const mockGalleryViewStore = {
  setCardVisibility: vi.fn(),
  isAutoScrolling: ref(false),
  isDetailViewActive: ref(false),
}
const mockInfiniteScrollStore = {
  fetchMore: vi.fn(),
}
const mockUiStore = {
...
const MOCK_PICSUM_IMAGE: PicsumImage = {
  id: '0', author: 'Mock Author', width: 100, height: 100, url: 'url', download_url: 'download_url'
};
...
describe('useImageObserver', () => {
  beforeEach(() => {
    ...
    mockGalleryViewStore.isDetailViewActive.value = false;
  });

  it('sets up an observer on mount', async () => {
    mount(TestHarness, { props: { imagesRef: ref([]), scrollContainerRef: ref(document.createElement('div')) } });
    await nextTick();
    expect(IntersectionObserver).toHaveBeenCalledOnce();
  });

  it('calls setCardVisibility when an element becomes visible', async () => {
    mount(TestHarness, { props: { imagesRef: ref([{ ...MOCK_PICSUM_IMAGE, id: '1' }]), scrollContainerRef: ref(document.createElement('div')) } });
    await nextTick();
    const mockEntry = {
  ...
  it('calls fetchMore when a trigger card is intersected on a touch device', async () => {
    ...
    const images = Array.from({ length: 20 }, (_, i) => ({ ...MOCK_PICSUM_IMAGE, id: `${i}` }));
    const triggerIndex = 12;

    mount(TestHarness, { props: { imagesRef: ref(images), scrollContainerRef: ref(document.createElement('div')) } });
    await nextTick();
