...
export const useGalleryViewStore = defineStore('gallery-view', () => {
  ...
  const visibilityStatusText = computed(() => {
    ...
    const currentImages = imageIds.map(id => imageStore.findImageById(id)).filter(Boolean) as PicsumImage[];
  ...
  function preloadNeighboringImages(currentId: string) {
    ...
    if (uiStore.isTouchDevice) {
      ...
      const indexOnPage = paginationStore.pages[pageOfCurrentImage]?.ids.findIndex(id => id === currentId);
      if (indexOnPage === -1) return;
      
      if (indexOnPage >= paginationStore.pages[pageOfCurrentImage].ids.length - IMAGE_PRELOAD_OFFSET_DESKTOP) {
        const nextPage = pageOfCurrentImage + 1;
        if ((!paginationStore.totalPages || nextPage <= paginationStore.totalPages) && !preloadedInDetailView.value.has(nextPage)) {
